...
def process_reals(x, lod, mirror_augment, drange_data, drange_net):
  with tf.name_scope('ProcessReals'):
    ...
    with tf.name_scope(
        'FadeLOD'):  # Smooth crossfade between consecutive levels-of-detail.
      s = tf.shape(x)
...
class TrainingSchedule:
  ...
  def __init__(
    ...
    self.resolution = 2**(
        training_set.resolution_log2 - int(np.floor(self.lod)))
    ...
    if self.resolution in max_minibatch_per_gpu:
      self.minibatch = min(
          self.minibatch,
          max_minibatch_per_gpu[self.resolution] * config.num_gpus)
...
def train_progressive_gan(
  ...
  G_opt = tfutil.Optimizer(
      name='TrainG', learning_rate=lrate_in, **config.G_opt)
  D_opt = tfutil.Optimizer(
      name='TrainD', learning_rate=lrate_in, **config.D_opt)
...
>>>>>>> 8ef4a97162e04c75167f889e92e373ac564f65e2
