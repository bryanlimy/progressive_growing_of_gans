...
class TFRecordDataset:
  ...
  def get_random_labels_np(self, minibatch_size):  # => labels
    if self.label_size > 0:
      return self._np_labels[np.random.randint(
          self._np_labels.shape[0], size=[minibatch_size])]
    else:
...
class SyntheticDataset:
  ...
  def get_minibatch_tf(self):  # => images, labels
    with tf.name_scope('SyntheticDataset'):
      ...
      images = self._generate_images(self._tf_minibatch_var, self._tf_lod_var,
                                     shape)
  ...
  def _generate_images(self, minibatch, lod,
                       shape):  # to be overridden by subclasses
...
def load_dataset(class_name='dataset.TFRecordDataset',
                 data_dir=None,
                 verbose=False,
                 **kwargs):
