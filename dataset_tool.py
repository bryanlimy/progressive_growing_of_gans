  ...
  def add_image(self, img):
    ...
    for lod, tfr_writer in enumerate(self.tfr_writers):
      if lod:
        ...
        img = (img[:, 0::2, 0::2] + img[:, 0::2, 1::2] + img[:, 1::2, 0::2] +
               img[:, 1::2, 1::2]) * 0.25
...
def create_celebahq(tfrecord_dir,
                    celeba_dir,
                    delta_dir,
                    num_threads=4,
                    num_tasks=100):
  print('Loading CelebA from "%s"' % celeba_dir)
  expected_images = 202599
  if len(glob.glob(os.path.join(celeba_dir, 'img_celeba',
                                '*.jpg'))) != expected_images:
    error('Expected to find %d images' % expected_images)
  with open(
      os.path.join(celeba_dir, 'Anno', 'list_landmarks_celeba.txt'),
      'rt') as file:
    landmarks = [[float(value)
  ...
  img = np.array(
      PIL.Image.open(os.path.join(celeba_dir, 'img_celeba', '000001.jpg')))
  md5 = hashlib.md5()
  md5.update(img.tobytes())
  if md5.hexdigest() != '9cad8178d6cb0196b36f7b34bc5eb6d3':
    error(
        'create_celebahq requires libjpeg version 8d')  # conda install jpeg=8d
  ...
  def process_func(idx):
    ...
    # Transform.
    img = img.transform((4096, 4096), PIL.Image.QUAD, (quad + 0.5).flatten(),
                        PIL.Image.BILINEAR)
...
def execute_cmdline(argv):
  ...
  parser = argparse.ArgumentParser(
      prog=prog,
      description=
      'Tool for creating, extracting, and visualizing Progressive GAN datasets.',
